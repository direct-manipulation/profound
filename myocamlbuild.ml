...
let needs_refresh () =
  let open Unix in
  if Sys.file_exists "version.ml" then
    let st_b = stat "myocamlbuild.ml" in
    let st_v = stat "version.ml" in
    st_b.st_mtime >= st_v.st_mtime
  else true

let make_version () =
  if needs_refresh () then begin
    Printf.printf "Regenerating version.ml... %!" ;
